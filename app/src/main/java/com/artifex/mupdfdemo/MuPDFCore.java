...
public class MuPDFCore
{
	...
	public MuPDFCore(Context context, String filename) throws IOException
	{
		globals = openFile(filename);
		if (globals == 0)
		{
			throw new IOException(String.format(context.getString(R.string.cannot_open_file_Path), filename));
		}
	...
	public MuPDFCore(Context context, byte buffer[], String magic) throws IOException {
		fileBuffer = buffer;
		globals = openBuffer(magic != null ? magic : "");
		if (globals == 0)
		{
			throw new IOException(context.getString(R.string.cannot_open_buffer));
		}
		file_format = fileFormatInternal();
		isUnencryptedPDF = isUnencryptedPDFInternal();
